...
pub trait PartExt: 'static {
    ...
    fn get_content_description(&self) -> Option<glib::GString>;
    ...
    fn get_content_location(&self) -> Option<glib::GString>;

    #[doc(alias = "g_mime_part_get_content_md5")]
    fn get_content_md5(&self) -> Option<glib::GString>;
...
impl<O: IsA<Part>> PartExt for O {
    ...
    fn openpgp_encrypt(&self, sign: bool, userid: Option<&str>, flags: EncryptFlags, recipients: &[&str]) -> Result<(), glib::Error> {
        unsafe {
            let mut error = ptr::null_mut();
            let _ = ffi::g_mime_part_openpgp_encrypt(self.as_ref().to_glib_none().0, sign.to_glib(), userid.to_glib_none().0, flags.to_glib(), recipients.to_glib_none().0, &mut error);
