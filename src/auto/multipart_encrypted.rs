...
impl MultipartEncrypted {
    ...
    pub fn encrypt<P: IsA<CryptoContext>, Q: IsA<Object>>(ctx: &P, entity: &Q, sign: bool, userid: Option<&str>, flags: EncryptFlags, recipients: &[&str]) -> Result<Option<MultipartEncrypted>, glib::Error> {
        unsafe {
            ...
            let ret = ffi::g_mime_multipart_encrypted_encrypt(ctx.as_ref().to_glib_none().0, entity.as_ref().to_glib_none().0, sign.to_glib(), userid.to_glib_none().0, flags.to_glib(), recipients.to_glib_none().0, &mut error);
