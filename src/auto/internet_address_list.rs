...
pub trait InternetAddressListExt: 'static {
    ...
    fn encode(&self, options: Option<&mut FormatOptions>, str: &mut glib::String);
...
impl<O: IsA<InternetAddressList>> InternetAddressListExt for O {
    ...
    fn encode(&self, options: Option<&mut FormatOptions>, str: &mut glib::String) {
        unsafe {
            gmime_sys::internet_address_list_encode(self.as_ref().to_glib_none().0, options.to_glib_none_mut().0, str.to_glib_none_mut().0);
        }
    }
