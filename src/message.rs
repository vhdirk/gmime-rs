...
glib_wrapper! {
    pub struct Message(Object<ffi::GMimeMessage, ffi::GMimeMessageClass, MessageClass>) @extends Object;
...
impl<O: IsA<Message>> MessageExt for O {
    fn add_mailbox(&self, type_: AddressType, name: &str, addr: &str) {
        unsafe {
            ffi::g_mime_message_add_mailbox(self.as_ref().to_glib_none().0, type_.to_glib(), name.to_glib_none().0, addr.to_glib_none().0);
        }
    }
    ...
    fn get_addresses(&self, type_: AddressType) -> Option<InternetAddressList> {
        unsafe {
            from_glib_none(ffi::g_mime_message_get_addresses(self.as_ref().to_glib_none().0, type_.to_glib()))
        }
    }
    ...
    fn get_autocrypt_gossip_headers(&self, now: &glib::DateTime, flags: DecryptFlags, session_key: &str) -> Result<AutocryptHeaderList, Error> {
        unsafe {
            ...
            let ret = ffi::g_mime_message_get_autocrypt_gossip_headers(self.as_ref().to_glib_none().0, now.to_glib_none().0, flags.to_glib(), session_key.to_glib_none().0, &mut error);
    ...
    fn split(&self, max_size: usize) -> Vec<Message> {
        unsafe {
            ...
            let parts = ffi::g_mime_message_partial_split_message(self.as_ref().to_glib_none().0, max_size, &mut n_parts);
